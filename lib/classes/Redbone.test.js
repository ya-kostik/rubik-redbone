...
function initApp(port, autoStart = true) {
  ...
  config.configs.http.socket = { redbone: { rpc: true } };
...
test('Add the Redbone\'s kubik and up app', async (done) => {
  ...
  redbone.use(async (client, action) => {
    try {
      expect(client.rubik).toBe(app);
  ...
  const clientConnection = clientIO('http://localhost:' + (startPort + 1));
  socket.on('connection', () => {
    clientConnection.emit(redbone.inputEvent, testAction);
  });
});
...
test('Use watchers', async (done) => {
  ...
  await app.up();
  expect(app.kubiks.get('http/socket/redbone')).toBe(redbone);
  const clientConnection = clientIO('http://localhost:' + startPort);
