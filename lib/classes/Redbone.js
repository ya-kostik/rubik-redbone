...
class Redbone extends Rubik.Kubik {
  constructor(watchersVolumes, typesVolumes) {
    ...
    this.hooks.afterVolumesRead = [];
    this.inputEvent = 'server-dispatch';
    this.outputEvent = 'client-dispatch';
    this.rpc = null;
  }
   ...
   */
  async _init() {
    const SocketIOConnector = RedboneLib.connectors.SocketIO;
    const connector = new SocketIOConnector(this.io, this.inputEvent, this.outputEvent);
    this.redbone = new RedboneLib(connector);
    this.redbone.use(this._attachAppMiddleware);
    if (this.config.rpc) {
      const rpc = new RedboneLib.extensions.RPC();
      this.rpc = rpc;
      this.redbone.extension(rpc);
    }
  ...
  use(extension) {
    if (isFunction(extension.afterVolumesRead)) {
      this.hooks.afterVolumesRead.push(extension.afterVolumesRead);
    }
    super.use(extension);
  }
}
